...
2. Snapshot all `*.journal` files and `config.toml` under `dataDir` into memory
3. Execute caller-provided `fn` (does the actual file writes)
4. If `fn` fails or `hledger check` fails: revert all snapshotted files from snapshot, return error
