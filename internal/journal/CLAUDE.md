...
- `UpdateTransaction(ctx, hledgerClient, dataDir, fid, description, newDate, comment, postings)` — replaces description, date, comment, and postings while preserving fid, status, and hidden meta. If `newDate` is empty, keeps the existing date.
