...
- `Restore(ctx, hash)` — hard-resets the working tree to the given commit. Preserves `float.key` and `ssh_host_key` across the reset (they are gitignored; `config.toml` is tracked in git and reverts normally).
...
The `.gitignore` excludes `float.key` (crypto secret). `config.toml` is tracked in git so all config mutations are versioned alongside journal changes. `Restore` re-writes `float.key` and `ssh_host_key` from memory after the hard reset so they are not affected by snapshot rollbacks. On startup, `New` updates the `.gitignore` for existing repos (handles migrations).
