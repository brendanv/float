...
test("accounts page - rename dialog input", async ({ page }) => {
  ...
  await page.evaluate(() => document.querySelector("vite-error-overlay")?.remove());
  await page.waitForTimeout(600);
  await page.getByRole("button", { name: "Rename assets:checking" }).click();
  await page.waitForSelector('[data-slot="dialog-content"]');
  await page.waitForTimeout(200);
  await expect(page.locator('[data-slot="dialog-content"] [role="combobox"]')).toContainText("assets:checking");
  const newNameInput = page.locator('#rename-new');
  await expect(newNameInput).toHaveValue("assets:checking");
